...
async function handleMemory(i: ChatInputCommandInteraction): Promise<void> {
  try {
    ...
    if (!content) {
      await reply(i, "Memory file is empty. I'll update it as we chat.");
      return;
    }
    const display =
      content.length > 1900
        ...
        : content;
    await reply(i, display);
  } catch {
    await reply(i, "Could not read memory file.");
  }
}
...
async function handleModel(
  ...
  if (!arg || arg.toLowerCase() === "reset" || arg.toLowerCase() === "default") {
    ...
    if (be?.getSettingsPresentation) {
      ...
      await reply(i, `**Model:** \`${formatModelLabel(activeModel)}\``);
    }
    return;
  }
  ...
  if (be?.resolveModel) {
    ...
    await reply(i, `Model set to \`${formatModelLabel(model)}\`.`);
  }
}
...
async function handlePulse(
  ...
  if (arg === "on" || arg === "enable") {
    enablePulse(chatId);
    registerChat(chatId);
    await reply(i, "🔔 Pulse enabled.");
    return;
  }
  if (arg === "off" || arg === "disable") {
    disablePulse(chatId);
    await reply(i, "🔔 Pulse disabled.");
    return;
  }
  ...
  if (intervalMs) {
    await reply(i, "Minimum interval is 5 minutes.");
    return;
  }
  await reply(i, "Use: /pulse on, /pulse off, /pulse 30m, /pulse 2h");
}
...
async function handleSettings(
  ...
  // Build components: model select menu, effort buttons, pulse toggle, done.
...
async function handlePlugins(i: ChatInputCommandInteraction): Promise<void> {
  ...
  if (plugins.length === 0) {
    await reply(i, "No plugins loaded.");
    return;
  }
