...
async function notifyUnauthorized(
  ...
  for (const adminId of access.adminUserIds) {
    try {
      ...
      const detail =
        type === "dm"
          ? `🚨 Unauthorized DM from ${senderName}${senderTag} [id:${userId}]`
          : type === "guild"
            ? `🚨 Unauthorized guild access: "${msg.guild?.name ?? guildId}" [id:${guildId}] by ${senderName}${senderTag}`
            : `🚨 Unauthorized channel access: #${(msg.channel as { name?: string }).name ?? channelId} in "${msg.guild?.name ?? guildId}" by ${senderName}${senderTag}`;
...
async function sendChunked(
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      ...
      logWarn(
        "bot",
        `Discord send failed: ${err instanceof Error ? err.message : err}`,
      );
    }
  }
  return ids;
...
/** Truncate helper re-exported for callers building inline text. */
export { truncate };
